...
parser.add_argument('--confounder_strength', '-c', type=float)
args = parser.parse_args()
################ Paths and other configs - Set these #################################
cub_dir = '/scratch/lhz209/nood/data/CUB_200_2011/'
places_dir = '/scratch/lhz209/nood/data/places'
output_dir = '/scratch/lhz209/nood/data/waterbirds_erm'
...
confounder_strength = args.confounder_strength    # Determines relative size of majority vs. minority groups
...
for split_idx, ids in enumerate([train_ids, val_ids, test_ids]):
    for y in (0, 1):
        # different from waterbirds in that val is same ratio as train, test is balanced
        # orig has val and test are balanced
        if split_idx in [0, 1]: # train and val
            if y == 0:
                pos_fraction = 1 - confounder_strength
            else:
                pos_fraction = confounder_strength
        else:  # test is balanced
            pos_fraction = .5
